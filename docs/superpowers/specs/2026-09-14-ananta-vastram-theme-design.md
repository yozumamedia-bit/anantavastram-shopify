...
Theme settings added to `settings_schema.json` under a new "Ananta Vastram" group: `av_display_font` (font_picker, default Rubik Mono One), `av_wordmark_font` (font_picker, default Cormorant Garamond 700), `av_logo` (image_picker, no default; see §1), `av_whatsapp_number`, `av_shipping_line`, `av_instagram_url`. Defaults land in `settings_data.json`. `theme.liquid` emits `font_face` declarations for the two new pickers (defaults `rubik_mono_one_n4`, `cormorant_garamond_n7`) and the custom properties `--av-display-family` and `--av-wordmark-family`.
