...
- `av-drop-rows` takes a `drop` metaobject reference; if blank it auto-selects by position (setting `auto_index`: 1 = newest, 2 = second newest, using the handle iteration below and skipping `closed` drops). Renders `av-drop-banner` then one `av-product-row` per product in the drop's collection.
...
- `av-drop-index` renders in two passes over the handle range so the current drop is always first: pass 1 renders the newest drop with `status = current` (and a non-empty collection) as the large band (7/5, ensemble image, chip "Current drop", counts, six tiles, "Enter the drop"); pass 2 renders every other drop, newest first, under a once-only "Past drops" head as the compact band (snippet `av-drop-band`) with chip "Pieces remaining", or outline chip "Closed" when `status = closed` or no stock remains, a state sentence keyed on remaining stock, then tiles. Exactly one drop should have `status = current` (setup.md says so); if several do, the newest wins and the rest render as past bands. Drops with an empty or missing collection are skipped. No commission content.
